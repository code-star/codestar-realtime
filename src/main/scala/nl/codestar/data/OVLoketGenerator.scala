...
class OVLoketGenerator(port: Int, envelopes: Iterable[String], url: String = "pubsub.besteffort.ndovloket.nl")(
...
//        val msgType    = msg.pop.toString
        ...
        val vehicleInfos = TrainLocations
          ...
          .mapValues(p =>
            VehicleInfo(p.latitude, p.longitude, Instant.ofEpochMilli(VehicleInfo.gpsTimeToMillis(p.gpsDatetime))))
